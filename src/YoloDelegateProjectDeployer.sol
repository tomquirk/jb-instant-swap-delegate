...
import {YoloDelegateDeployer} from "./YoloDelegateDeployer.sol";

/// @notice Deploys a project, or reconfigure an existing project's funding cycles, with a newly deployed Delegate attached.
contract YoloDelegateProjectDeployer is JBOperatable {
    /// @notice The contract responsible for deploying the delegate.
    YoloDelegateDeployer public immutable delegateDeployer;

    /// @param _delegateDeployer The delegate deployer.
    constructor(YoloDelegateDeployer _delegateDeployer, IJBOperatorStore _operatorStore) JBOperatable(_operatorStore) {
    ...
    function launchProjectFor(
        ...
        YoloDelegate _delegate =
            delegateDeployer.deployDelegateFor(projectId, _controller.directory());
        ...
        returns (uint256 configuration)
    {
        // Deploy the delegate contract.
        YoloDelegate _delegate =
            delegateDeployer.deployDelegateFor(_projectId, _controller.directory());
